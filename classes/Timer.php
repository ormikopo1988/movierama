...
class Timer
{
	...
	public static function stopSave(
		&$po_timers,
		$p_string
	)
	{
		self::stop();
		$po_timers[] = array( $p_string => self::get(self::MILLISECONDS) );
	}
	
	public static function stopSaveAndShowAll(
		&$po_timers,
		$p_string
	)
	{
		self::stop();
		$po_timers[] = array( $p_string => self::get(self::MILLISECONDS) );
		self::showTimers($po_timers);
	}
	
	public static function showTimers( $p_timersArray ) {
		foreach( $p_timersArray as $aTimerElement ) {
			foreach( $aTimerElement as $aName => $aTiming ) {
				echo "$aName: $aTiming<br>";
			}
		}
	}
}
