...
class VO_CtrlEvaluation {
	...
	public static function createEvaluationForm( WOOOF $wo, $whatType, $whatId ) {
		...
		$pageLocation='3';
		$browserTitle='MovieRama Evaluation Creation Page';
...
EOH
	...
	public static function viewEvaluations( WOOOF $wo, $whatId, $whatType ) {
		...
		$requestedAction='viewUncontroled';
		$pageLocation='3';
		$browserTitle='MovieRama Evaluations View Page';
		$wo = WOOOF::getWOOOF($pageLocation, $requestedAction, $wo);
		if ( $wo === FALSE ) { die('Failed to getWOOOF()'); }
		...
		if ( !$wo->hasContent($whatType) ) {
			$wo->logError(self::_ECP."5095 No value for type of object to see its evaluations");
			return false;
		}
...
EOH
	...
	public static function viewEvaluationResults( WOOOF $wo, $evaluationId ) {
	
		$requestedAction='viewUncontroled';
		$pageLocation='3';
		$browserTitle='MovieRama Evaluation Results Page';
		$wo = WOOOF::getWOOOF($pageLocation, $requestedAction, $wo);
		if ( $wo === FALSE ) { die('Failed to getWOOOF()'); }
		...
		if ( !$wo->hasContent($evaluationId) ) {
			$wo->logError(self::_ECP."5067 No value for evaluation id");
			return false;
		}
		
		$evaluationRow = $wo->db->getRow('evaluations', $evaluationId);
		if($evaluationRow === FALSE) { return false; }
		if($evaluationRow === NULL) {
			$wo->logError(self::_ECP."5068 No evaluation with this id found");
			return false;
		}
		
		$data = VO_Evaluation::getEvaluationResults($wo, $evaluationId, $wo->app->userId );
		if ( $data === FALSE ) { $wo->handleShowStopperError(); }
